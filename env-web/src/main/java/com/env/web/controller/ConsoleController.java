...
public class ConsoleController {
		...
		try {
		    ...
		    list.forEach(item -> {
				...
				for(int j = 0; j<item.getData().size(); j++) {
					...
					if(m.getYinziValue() == null || m.getYinziValue() == 0.000)
						continue;
					else
	...
	public ResponseApi shishiHeader() throws Exception {
		...
		e.setWidth(174);h.setWidth(90);b.setWidth(93);
		header.add(h);header.add(d);header.add(e);header.add(c);header.add(b);		
		api.success(header);
		return api;
	}
}
